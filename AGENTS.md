...
- The LangGraph `tools_node` wraps tool calls and emits English custom progress chunks before tool execution, such as `Querying database`, `Searching knowledge base`, `Submitting code for judging`, `Creating code file`, `Updating code file`, `Overwriting code file`, `Updating code file metadata`, `Checking current time`, and `Checking current user`.
- `app.agents.orchestrator.AGENT_TOOLS` is the shared entrypoint for business tools and currently includes `query_oj_database`, `search_knowledge_base`, `submit_code_for_judge`, `create_code_file`, `replace_code_file_content`, `replace_code_file_content_as_new`, `overwrite_code_file`, `update_code_file_metadata`, `get_current_time`, and `get_current_user_id`.
- `search_knowledge_base` describes the RAG knowledge base to the model as currently mainly containing information about the TenJudge platform.
- `query_oj_database` accepts one read-only `SELECT` / `WITH SELECT` query and returns JSON containing `success`, `columns`, `rows`, `row_count`, `truncated`, and `sql`.
...
- Database tool result limits are module constants in `app/tools/database.py`: `AGENT_DB_TOOL_MAX_ROWS`, `AGENT_DB_TOOL_MAX_FIELD_CHARS`, and `AGENT_DB_TOOL_MAX_RESULT_CHARS`.
...
- Database tool description documents known categorical values for problem visibility (`public`/`private`), user roles (`admin`/`super_admin`/`user`), Codeforces-style numeric problem difficulty ratings, and the fixed supported problem tag list.
- `query_oj_database` does not try to parse or structurally validate SQL in application code; it only normalizes whitespace and one trailing semicolon for wrapper compatibility. Read-only access and relation restrictions are enforced by the restricted database role, role-level default read-only setting, `agent_read` grants, role-level timeout, and result limits.
- `query_oj_database` wraps the normalized user SQL with a bounded literal `LIMIT` instead of a psycopg placeholder, so percent patterns in SQL string literals such as `ILIKE '%dp%'` are not parsed as client-side placeholders.
- `query_oj_database` catches SQL normalization and PostgreSQL query errors and returns `success=false` JSON with `error.type`, `error.message`, and optional `error.sqlstate`; missing `AGENT_DB_TOOL_DATABASE_URL` remains a fatal configuration error.
...
- RAG import directories are configured by `Settings.RAG_SKILL_DIR` and `Settings.RAG_KNOWLEDGE_DIR`; FastAPI startup calls `app.rag.importer.startup_rag()` after opening the database pool, imports configured directories, and rebuilds shared in-process BM25/vector retrievers.
- RAG embedding calls use `OpenAIEmbeddings.max_retries = 3` instead of a custom retry wrapper.
